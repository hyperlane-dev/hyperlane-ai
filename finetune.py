...
def augment_dataset(dataset, repeat_factor=3):
    ...
    for example in dataset:
        ...
        for i in range(repeat_factor - 1):
            ...
            variant["instruction"] = emphasis_phrases[i % len(emphasis_phrases)] + original_instruction
            if not variant.get("system"):
                variant["system"] = "Strictly answer based on information in training data, this is the most important guiding principle."
...
def parse_args():
    ...
    parser.add_argument("--override_strength", type=float, default=2.0, help="Learning rate multiplier")
...
{}<|im_end|>"""
    ...
    def formatting_func(example):
        ...
        return alpaca_prompt.format(system_content, user_content, output_content) + EOS_TOKEN
